    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        <div className="p-5 rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 shadow-lg hover:shadow-xl transition-shadow">
          <div className="text-xs font-medium text-green-600 mb-1">Uang Jajan Default</div>
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100 shadow-lg hover:shadow-xl transition-shadow">
          <div className="text-xs font-medium text-blue-600 mb-1">Sisa Hari Ini</div>
          <div className="text-2xl font-bold text-gray-800">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-pink-50 to-rose-50 border border-pink-100 shadow-lg hover:shadow-xl transition-shadow">
          <div className="text-xs font-medium text-pink-600 mb-1">Tujuan Aktif</div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        {activeGoal ? (
          <div className="space-y-4">
            ...
            <div className="text-center space-y-2">
              <div className="text-sm text-gray-600">
                Terkumpul <span className="font-bold text-green-600">{formatCurrency(activeGoal.savedAmount)}</span> dari{" "}
                <span className="font-bold text-gray-800">{formatCurrency(activeGoal.targetAmount)}</span>
              </div>
              <div className="inline-block px-4 py-2 bg-gradient-to-r from-green-100 to-emerald-100 rounded-lg">
                ...
                <div className="text-sm font-bold text-green-700">
                  {eta
                    ? `${new Date(eta.date).toLocaleDateString("id-ID")} (~${eta.days} hari)`
                    : "Belum ada estimasi"}
                </div>
              </div>
            </div>
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <div className="text-sm">Belum ada tujuan aktif. Buat tujuan di menu Tujuan!</div>
          </div>
        )}
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="px-4 py-2 bg-green-50 rounded-xl text-xs text-green-700 border border-green-100">
              💰 Rata-rata saat ini: <span className="font-bold">{formatCurrency(Math.round(getSavingsAverage14d()))}</span>
            </div>
          </div>
        </div>
        <div className="mt-6 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl">
          ...
          {activeGoal && (
            <div className="text-sm text-gray-700 mt-3 text-center p-3 bg-white rounded-lg shadow-sm">
              📅 Jika menabung <span className="font-bold text-green-600">{formatCurrency(simDailySaving)}</span> per hari, 
              estimasi tercapai dalam <span className="font-bold text-pink-600">{simDaysToTarget ?? "—"}</span> hari.
            </div>
          )}
        </div>
      </section>
      ...
      {!goals.allIds.length && (
        <div className="p-6 rounded-2xl bg-gradient-to-r from-pink-50 to-rose-50 border-2 border-pink-200 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-pink-700 mb-1">Belum Ada Tujuan</div>
              <div className="text-sm text-pink-600">Kamu belum punya tujuan. Ayo buat tujuan pertama di menu "Tujuan"!</div>
            </div>
          </div>
        </div>
      )}
